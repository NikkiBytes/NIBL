...
import os
import shutil
subjects=glob.glob("/projects/niblab/bids_projects/Experiments/Bevel/data/derivatives/sub-*")
for sub in subjects:
    ...
    if not regstandard_dirs:
        ...
        for dir in regstandard_dirs:
            ...
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", dir)
            shutil.rmtree(dir)
    mat_path = os.path.join(sub, "func/Analysis/feat1/*run*.feat/reg/*.mat")
    ...
    FEAT_PATH = os.path.join(sub, "func/Analysis/feat1/*run*.feat")
