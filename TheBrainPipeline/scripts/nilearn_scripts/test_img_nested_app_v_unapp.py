...
#make a list of the files to concat-testing with cs001, cs002, cs004, cs005, cs006, cs007, cs008, cs009
test_func = glob.glob(os.path.join(basepath,'level1_grace_edit','cs00*++.feat','filtered_func_data.nii.gz'))

#load in all the files from the glob above, then convert them from nifti1 to nifti2
ni2_funcs = (nib.Nifti2Image.from_image(nib.load(func)) for func in test_func)
...
outfile=os.path.join(basepath,'concatenated_imagine_8test.nii')
#write the file
ni2_concat.to_filename(outfile)

#convert power ROI mask to nifti-2
img_mask_ni1 = os.path.join('/projects','niblab','scripts','nilean_stuff','power_roimask_4bi.nii.gz')
imag_mask_ni2 = nib.Nifti2Image.from_image(nib.load(img_mask_ni1))

# ---STEP 2---
#load & prepare MRI data
#load, fxnl, anatomical, & mask for plotting
fmri_subjs=os.path.join(basepath, 'concatenated_imagine_8test.nii')
average_ana = os.path.join(basepath,'CS_avg_mprage_image.nii.gz')
imag_mask = imag_mask_ni2
