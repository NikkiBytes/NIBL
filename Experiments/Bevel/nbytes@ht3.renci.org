    ...
    for sub in sub_dict:
        ...
        for func_input in glob.glob(os.path.join(func_keyword)):
         ...
         # set input/output path
            ...
            outhtml = os.path.join(output,'bold_motion_QA_test_%s.html'%(datestamp))
         ...
         # set comparison param
           ...
           # making a plot and putting it in the motion assessment directory we made above
            ...
            os.system("fsl_motion_outliers -i %s  -o %s/%s_confound.txt  --nomoco  --fd --thresh=0.9 -p motion_assessment/fd_plot -v > %s/%s_outlier_output.txt"%(fsl_input, output,task,output, task))
